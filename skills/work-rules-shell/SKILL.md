...
- **A quoted heredoc is not safe for a body with apostrophes either (measured 2026-09-15).** A `<<'EOF'` Python script whose strings held English prose (`learner's`, `operator's`) died with `unexpected EOF while looking for matching '`, before Python ran, while shorter heredocs in the same session passed. Same boundary as above. For any script longer than a few lines, Write it to the scratchpad and run the file (SH-1), and build newlines in it as `chr(10)`.
