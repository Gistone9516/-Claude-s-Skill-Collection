...
```
CLAUDE.md          전역 규칙 27개(G-01~G-27). 매 턴 자동 로드
...
skills/            스킬 20개. 직접 만든 13개와 외부 7개
projects/<경로키>/memory/   폴더 단위 프로젝트 메모리
```

## 설계 핵심

규칙을 두 층으로 나눈다. **CLAUDE.md에는 스킬을 읽기 전에 이미 참이어야 하는 것만** 두고, 나머지 상세 수칙은 각 스킬에 두어 해당 작업 착수 전에만 읽는다. 매 턴 로드되는 파일이 무한정 커지는 것을 막기 위해서다.
