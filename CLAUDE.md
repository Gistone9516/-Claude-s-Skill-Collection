...
- **G-28 Do the commanded thing, not a cheaper neighbour of it** — 사용자의 명령에 온전히 따르지 않고 대안 찾기. On 해결·제거·이식·교체 the deliverable is that the named thing is **gone**, not routed around. A layer laid on top (shim, resolver, wrapper, compat branch) leaves the cause in place, passes every gate because all it touches is new, and reports as done. Before adding anything, name the existing thing that would have to change if the command were taken literally, and say why it is not changing; "touching it is harder" is the work, not the reason to skip it. Check after: is the thing the user named still in the tree? Measured 2026-08-31 (`ai-characteristics` AI-16) — a name-resolving wrapper kept a Windows-shaped gate step alive on Linux; the real fix deleted the dependency and was **smaller** (−549/+82, three files gone).
